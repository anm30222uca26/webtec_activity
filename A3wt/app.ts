// Part B: TypeScript Definitions
type StudentStatus = "Undergraduate" | "Graduate";
...
const calcPerc = (s: StudentProfile): string => {
    let score = 0;
    if (s.fullName) score += 25;
    if (s.level) score += 25;
...
contactForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    const vName = (document.getElementById('vName') as HTMLInputElement).value;
    const vEmail = (document.getElementById('vEmail') as HTMLInputElement).value;
